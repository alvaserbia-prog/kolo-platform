...
*Izmene u verziji 3.7.6: dopunjen član 7 (Kolačići) — opisani analitički kolačići trećih lica (Google Analytics i Microsoft Clarity), njihova svrha, pravni osnov (pristanak korisnika) i mehanizam davanja/povlačenja pristanka; dopunjen član 8 (Primaoci i obrađivači) konkretnim obrađivačima (hosting, email, analitika); preciziran član 9 (Prenos van Srbije) jer pojedini obrađivači posluju van Republike Srbije.*
...
Tabla zahteva za jemstvo. Tekst predstavljanja iz pododeljke 4.8 dostupan je svim prijavljenim korisnicima Platforme, uključujući neverifikovane. Kontakt podaci iz iste pododeljke dostupni su isključivo verifikovanim korisnicima i prikazuju se tek po izričitom otkrivanju koje se beleži u evidenciji pristupa iz člana 5 ove politike. Tekst i kontakt podaci nisu dostupni licima koja nisu prijavljena na Platformu i ne indeksiraju se od strane pretraživača.
...
Platforma koristi dve vrste kolačića:

**1. Neophodni kolačići.** Neophodni su za funkcionisanje Platforme — prijava, održavanje sesije, bezbednosne postavke. Ovi kolačići nisu podložni pristanku jer su neophodni za pružanje usluge i ne koriste se za praćenje ili profilisanje.

**2. Analitički kolačići (trećih lica).** Uz prethodni pristanak korisnika, Platforma koristi analitičke alate trećih lica radi razumevanja korišćenja i poboljšanja sajta:

- **Google Analytics** (Google Ireland Limited / Google LLC) — agregatna statistika poseta i korišćenja stranica. IP adresa se anonimizuje (`anonymize_ip`).
- **Microsoft Clarity** (Microsoft Corporation) — analiza korišćenja stranica (heatmape i snimci interakcije sa stranicom u pseudonimnom obliku) radi otkrivanja problema u korišćenju.

Pravni osnov za analitičke kolačiće je **pristanak korisnika** (čl. 12 st. 1 t. 1 ZZPL-a). Ovi kolačići se **ne učitavaju dok korisnik ne da pristanak** putem banera za kolačiće koji se prikazuje pri prvoj poseti. Korisnik može odbiti analitičke kolačiće bez ikakvih posledica po korišćenje Platforme, a dati pristanak može povući u svakom trenutku (brisanjem lokalno sačuvane odluke u pregledaču, čime se baner ponovo prikazuje). Pored toga, Platforma koristi i analitiku bez kolačića (agregatni Vercel Analytics) koja ne postavlja kolačiće niti omogućava identifikaciju pojedinca.

Korisnik dodatno može upravljati kolačićima i kroz podešavanja svog internet pregledača.
...
Ako Fondacija angažuje treća lica za održavanje infrastrukture i pružanje pojedinih usluga, ta lica su obrađivači podataka u smislu čl. 45 ZZPL-a. Odnos između Fondacije i obrađivača uređuje se ugovorom o obradi podataka.

Podaci mogu biti dostupni sledećim primaocima:

- **Hosting i isporuka aplikacije** — Vercel Inc. (SAD) i baza podataka kod Neon Inc. (SAD) — isključivo u svrhu tehničkog funkcionisanja Platforme, na osnovu ugovora o obradi podataka.
- **Email servis** — Resend (Plus Five Five, Inc., SAD) — isključivo za dostavljanje sistemskih obaveštenja (npr. oporavak lozinke) i internih upozorenja Fondacije.
- **Analitika (samo uz pristanak korisnika)** — Google (Google Ireland Limited / Google LLC) i Microsoft Corporation — za statistiku korišćenja, u skladu sa članom 7.
...
Pojedini obrađivači iz člana 8 (hosting, baza podataka, email servis i, uz pristanak, analitika) posluju van teritorije Republike Srbije, pretežno u Sjedinjenim Američkim Državama. Usled toga se podaci mogu preneti van Srbije.

Prenos se vrši uz primenu odgovarajućih zaštitnih mera u skladu sa čl. 65–69 ZZPL-a. Fondacija obezbeđuje da prenos podataka u treće zemlje bude zasnovan na adekvatnom nivou zaštite — odlukom o adekvatnosti, standardnim ugovornim klauzulama (SCC) ili drugim odgovarajućim merama zaštite predviđenim zakonom. Izbor provajdera infrastrukture uzima u obzir lokaciju servera i primenjiv pravni okvir za zaštitu podataka u jurisdikciji u kojoj se serveri nalaze. Analitički podaci se prenose tek nakon pristanka korisnika, uz prethodno opisane mere.
