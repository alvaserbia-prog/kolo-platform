...
| **Verzija DPIA** | 3.6.0 |
| **Datum izrade** | 23.05.2026. |
| **Predmet procene** | KOLO sistem — Faza 1, aktivne radnje obrade br. 1–9; neaktivne radnje obrade br. 10–11 (Moduli 3 i 4) |
| **Povezani dokumenti** | Pravilnik o KOLO sistemu (v3.6.2), Politika privatnosti (v3.6.1), Registar radnji obrade (v3.6.0), Pravilnik o hijerarhiji akata (v3.5.0), Statut (v3.6.0), Whitepaper (v3.6.0) |
| **Sledeća revizija** | Pre aktiviranja Modula 3 ili 4, ili najkasnije 12 meseci od izrade |
...
Sistem u Fazi 1 obuhvata jedanaest radnji obrade definisanih Registrom radnji obrade (v3.6.0). Radnje 1–9 su aktivne; radnje 10–11 su neaktivne do aktiviranja Modula 3 i 4.
...
| R10 — Narušavanje integriteta evidencije | 1 | 4 | 4 | Retroaktivna promena zapisa bi ugrozila zajedničko dobro. Mere: svaki zapis vremenski označen i vezan za prethodno stanje, softverska arhitektura sprečava retroaktivnu promenu, redovne provere konzistentnosti. |
...
Pseudonimizacija evidencije — zapisi su vezani za pseudonime bez centralizovane tabele za povezivanje sa identitetima. Razdvajanje podataka — identifikacioni podaci (pseudonim, email) čuvaju se odvojeno od obračunskih podataka (evidencija doprinosa, POEN stanja). Šifrovanje podataka u prenosu — TLS enkripcija minimalno verzija 1.2 za svu komunikaciju. Šifrovanje podataka u mirovanju — enkripcija na nivou skladištenja sa razdvojenim čuvanjem ključeva. Integritet lanca evidencije — svaki zapis je vremenski označen i vezan za prethodno stanje; retroaktivna promena nije moguća bez narušavanja integriteta. Hashiranje lozinki — lozinke se čuvaju isključivo u hashovanom obliku. Evidencija pristupa — svaki pristup podacima se beleži u zaštićenom formatu koji se ne može retroaktivno menjati.
...
## 5.6. Rezidualni rizik

Nakon primene svih navedenih mera, rezidualni rizik se ocenjuje kao prihvatljiv. Najviši rezidualni rizici su R1 (neovlašćen pristup), R2 (reidentifikacija), R5 (tabla zahteva) i R8 (prekogranični prenos), svi na srednjem nivou (5–8). Ovi rizici se dodatno umanjuju kontinuiranim nadzorom, redovnim testiranjem i ažuriranjem mera.
...
## 8.1. Modul 3 — Socijalni programi

Aktiviranjem Modula 3 u skladu sa čl. 57 Pravilnika, sistem uvodi obradu posebnih kategorija podataka — status roditelja, starosna dob, invaliditet, studentski status (čl. 17 ZZPL-a). Pravni osnov je izričit pristanak korisnika (čl. 17 st. 2 t. 1 ZZPL-a). Fondacija ne čuva kopije podnesene dokumentacije — ostaje samo minimalni zapis o pripadnosti grupi i datum verifikacije.

Dodatni rizici koje ovaj modul uvodi: obrada posebnih kategorija podataka nosi inherentno veći rizik za prava i slobode lica; mogućnost diskriminacije na osnovu statusa; potreba za pojačanom kontrolom pristupa i odvojenim čuvanjem. Aktiviranje zahteva prethodno ažuriranje ove DPIA sa detaljnom procenom novih rizika i dodatnih mera zaštite pre početka obrade.
...
Identifikovano je deset rizika po prava i slobode lica na koja se podaci odnose. Nijedan rizik nije ocenjen kao visok. Četiri rizika su na srednjem nivou (R1, R2, R5, R8), a šest na niskom (R3, R4, R6, R7, R9, R10).
...
Ova DPIA se ažurira u sledećim slučajevima: pre aktiviranja Modula 3 (Socijalni programi), pre aktiviranja Modula 4 (Deca), pre uvođenja mehanizma table zahteva za jemstvo koji bi proširio krug vidljivih podataka, pri značajnoj promeni infrastrukture ili obrađivača, pri promeni pravnog okvira, ili najkasnije 12 meseci od datuma izrade.
