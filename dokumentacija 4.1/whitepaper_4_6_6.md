...
Integritet sistema počiva na modelu dokaza stvarnosti — lancu potvrda zasnovanom na ličnom poznavanju — u kome postojeći učesnici potvrđuju stvarnost, jedinstvenost i kontinuitet novih korisnika. Ovaj model je svesna dizajnerska odluka koja minimizira prikupljanje ličnih podataka, u skladu sa principom minimizacije podataka ugrađenim u sistem.
...
Mehanika otpisa. Korisnik koji upiše ZRNO pri jednom obračunskom koeficijentu a otpiše ga pri drugom, dobija drugačiji broj POEN-a od onog koji je iskoristio kao osnov upisa — u oba smera, jer koeficijent raste od upisa novih zapisa POEN-a a pada od njihovog poništenja (odeljak 6.4). Četiri elementa razbijaju kvalifikaciju te razlike kao prinosa: POEN-i evidentirani otpisom nemaju eksternu imovinsku vrednost — ne mogu se konvertovati u novac, izneti iz sistema ni monetizovati (princip nekonvertibilnosti); promena koeficijenta nije garantovana — proizlazi iz stanja evidencije celokupne zajednice, ne iz napora trećih lica u smislu investicionog ugovora; sopstveni upis i otpis koeficijent uopšte ne pomeraju, pa nosilac nema ni instrument ni trenutak kojim bi razliku ostvario; ne postoji izdavalac koji obećava rast koeficijenta niti institucija koja isplaćuje razliku. Dodatno, struktura sistema uspostavlja strukturni izbor koji ograničava čisto pasivno držanje: nosilac koji želi upravljačku korist mora da aktivira ZRNO, čime gubi mogućnost otpisa; nosilac koji želi obračunsku fleksibilnost ne može istovremeno da glasa.
...
Koeficijent pomeraju isključivo promene ukupnog broja evidentiranih POEN-a — naviše i naniže.

Kad god protokol upiše nove zapise POEN-a — kroz korisnički doprinos (donacije, pokroviteljstvo, operativni doprinos, verifikacija), rast krugova i zadruga, socijalne programe ili osnivački doprinos — brojilac formule raste, bez obzira na to u čijem se zapisu novi POEN-i evidentiraju. To menja obračunski koeficijent naviše.

Kad se evidentirani POEN-i ponište — prestankom svojstva korisnika, poništenjem po iskorišćenju u kolektivnoj nabavci, otpisom po raskinutom prijateljstvu u dečjem prostoru ili drugim osnovom utvrđenim pravilnikom — brojilac opada. To menja obračunski koeficijent naniže. Koeficijent, dakle, nije monoton i nije predodređen da raste.

Razmena dobara i usluga ne utiče na koeficijent jer redistribuira postojeće POEN-e bez promene ukupnog broja.

Upis i otpis ZRNA takođe ne pomeraju koeficijent, i to je posledica same formule, a ne pravila koje bi se moglo izmeniti. Pri upisu se imenilac umanjuje za upisana ZRNA, ali se brojilac istovremeno umanjuje za POEN-e koji su tim upisom vraćeni protokolu i izašli iz opticaja — a taj broj je upravo proizvod upisanih ZRNA i važećeg koeficijenta, pa količnik ostaje isti. Pri otpisu oba člana rastu po istom odnosu. Nosilac ZRNA svojim postupanjem ne može da pomeri sopstveni koeficijent ni u jednom smeru.

Svi ovi efekti su posledica aktivnosti korisnika u sistemu. Nijedan pojedinačni korisnik ne kontroliše koeficijent. Nijedna institucija ne kontroliše koeficijent. Koeficijent je izračunata vrednost koja proizlazi iz ukupnog stanja evidencije svih korisnika u sistemu.
...
Korisnik koji doprinosi sistemu u ranoj fazi — kad je koeficijent nizak — upisuje ZRNO uz manje evidentiranih POEN-a nego korisnik koji to isto čini u kasnijoj fazi sa višim koeficijentom. Pozicija ranog učesnika time odražava njegov doprinos u fazi kad je doprinos bio najvredniji za uspostavljanje sistema. To je posledica uvažavanja ranijeg doprinosa zajedničkom dobru, a ne podsticaj za sticanje položaja: broj POEN-a potreban za upis izračunava protokol iz stanja evidencije, ne odluka kojom bi se rano učešće nagrađivalo, i nijedno lice ne obećava da će taj broj kasnije biti veći.

Pravilo od jedan procenat stanja po obračunskom periodu (odeljak 6.2) ograničava brzinu kojom se raspoloživa ZRNA preuzimaju — čak i kad veliki broj korisnika istovremeno ispuni uslove za upis, ukupan obim upisa po periodu je ograničen na jedan procenat ukupnog stanja svih kvalifikovanih korisnika. Time se čuva pristup kasnijih učesnika i koeficijent ostaje manje osetljiv na pojedinačne upise novih zapisa POEN-a, jer se broj raspoloživih ZRNA ne iscrpljuje naglo.
...
Svaki korisnik KOLO sistema mora da potvrdi svoju stvarnost, jedinstvenost i kontinuitet kroz model verifikacije zasnovan na ličnom poznavanju — lanac potvrda u kome postojeći učesnici potvrđuju nove. Model ne zahteva prikupljanje ličnih dokumenata. Svaki korisnik ima indeks stvarnosti (0–100%) koji određuje obim pristupa funkcijama sistema i verifikacioni kapacitet.
...
Za dinarski tok važe odvojeni mehanizmi identifikacije: verifikacija donatora se obezbeđuje kroz bankovni sistem — fondacija prima donacije sa verifikovanih bankovnih računa; verifikacija pokrovitelja (pravnih lica i preduzetnika) se obezbeđuje na osnovu ugovora o donaciji sa fondacijom. Ovi mehanizmi se odnose na identifikaciju za potrebe finansijskog toka fondacije, ne na dokaz stvarnosti korisnika u smislu lanca potvrda.

## Neverifikovani korisnik

Neverifikovani korisnik je lice registrovano na platformi čija stvarnost još nije potvrđena kroz lanac potvrda. Može da pregleda sistem i upozna se sa pravilima, ali nema pristup razmeni, evidenciji POEN-a ni doniranju.

Prelaz u status verifikovanog korisnika odvija se kad postojeći verifikovani korisnik potvrdi stvarnost, jedinstvenost i kontinuitet novog korisnika kroz lanac potvrda, čime korisnik stiče indeks stvarnosti od najmanje 10% i pun pristup sistemu.

## Verifikovani korisnik

Verifikovani korisnik je lice čija je stvarnost potvrđena kroz lanac potvrda i čiji je indeks stvarnosti najmanje 10%. Verifikovani korisnik razmenjuje dobra i usluge sa drugim korisnicima unutar sistema. Doprinosi zajedničkom dobru kroz aktivnosti čiji se doprinos evidentira u POEN-ima. Može da donira dinarska sredstva fondaciji. Može da učestvuje u krugovima i zadrugama kad se ti moduli aktiviraju (poglavlje 9).
...
Protokol ne evidentira osnivački doprinos odjednom, nego ga vezuje za rast sistema — evidentira ga u koracima fiksnog iznosa, prema kumulativnom rastu ukupnog broja POEN-a u sistemu. Ovo je dizajnerska odluka. Svaki upis novih zapisa POEN-a pomera obračunski koeficijent (odeljak 6.3); evidentiranje celog iznosa odjednom proizvelo bi nagli skok koeficijenta, dok vezivanje za rast sistema čini da koeficijent isti nivo dostigne srazmerno rastu. Kanal prati rast kakav god da je njegov tempo: ako ukupan broj POEN-a poraste naglo, koraci se evidentiraju istom brzinom kojom je rast nastupio.

Pošto je korak fiksan, uticaj jednog koraka na koeficijent opada kako sistem raste. Udeo zbirno evidentiranog osnivačkog doprinosa u ukupnom broju POEN-a je druga veličina i ona ne opada: i pragovi i korak izraženi su u fiksnim iznosima, a POEN evidentiran kroz ovaj kanal ulazi u ukupan broj, pa zbirni osnivački doprinos čini između približno jedne petine i približno jedne četvrtine ukupnog broja evidentiranih POEN-a kroz ceo vek kanala — u trenutku zatvaranja približno 24%. Taj udeo ne nosi imovinsko pravo i ne predstavlja potraživanje; krug osnivača je zatvoren i ne može se proširiti, a kanal se po dostizanju gornje granice zatvara trajno.

Gornja granica je mera doprinosa koji je prethodio sistemu: osnivači su u projektovanje, izradu i pravnu i organizacionu pripremu uložili sopstvena novčana sredstva i sopstveno vreme, u periodu u kome nijedan kanal evidentiranja nije radio. Taj rad se ne može evidentirati kroz operativni doprinos ni naknadno — dnevni limit tog kanala iznosi 10% ukupnog broja evidentiranih POEN-a, pa je dok je taj broj nula i limit nula.

Rezultat osnivačkog rada — softver, protokol i dokumentacija sistema — ulazi u zajedničko dobro pod licencama iz poglavlja 3: softver pod AGPL-3.0, sadržaj pod CC BY-SA 4.0. Osnivači po osnovu tog rada ne stiču isključivo pravo na rezultat; on je pod istim uslovima dostupan svim licima, a licence se ne mogu zameniti restriktivnijim.
...
Sistem primenjuje limit od 10% ukupnog broja POEN-a evidentiranih u sistemu po obračunskom periodu na zbir koji se u tom periodu može evidentirati kroz operativni doprinos i programe podrške zajedno. Taj limit nije gornja granica ukupnog broja POEN-a nego ključ kojim se emisija tog perioda deli kada zbir traženih iznosa premaši limit: svi iznosi se tada srazmerno umanjuju, a neevidentirani višak se ne prenosi u naredni period. Ukupan broj POEN-a nije ograničen unapred određenim brojem — prati broj učesnika i njihovu uključenost, jer se svaki zapis vezuje za doprinos određenog lica, i nije jednosmeran, jer ga poništenja zapisa umanjuju. Ovaj parametar podleže promeni kroz procese upravljanja opisane u poglavlju 10. Procedura prijave, izvršenja i verifikacije definisana je u Pravilniku.
...
Širenje na nove regije zahteva prilagodbe u više dimenzija: pravni okvir ciljne jurisdikcije (posebno u pogledu zaštite podataka, poreskog tretmana i statusa fondacije), jezička lokalizacija platforme, uspostavljanje lokalnog lanca potvrda za dokaz stvarnosti i eventualno formiranje lokalnih zadruga (Modul 2) kao organizacionih jedinica na novoj teritoriji.
...
U drugoj fazi, Gornje Kolo postaje upravno telo sistema. Gornje Kolo nastaje automatski sa aktivacijom ZRNA — čim prvi korisnici upišu ZRNO po pravilima iz poglavlja 6, oni čine Gornje Kolo. Nema zasebnog koraka aktivacije, nema dodatnih preduslova: milion POEN-a je prag koji istovremeno aktivira ZRNO i uspostavlja Gornje Kolo. Jedan prag, jedan prelaz. Gornje Kolo čine svi nosioci ZRNA. Odlučuje o pravilima protokola, o aktiviranju i deaktiviranju modula i o svim pitanjima koja utiču na zajedničko dobro, osim pitanja koja su izuzeta iz njegove nadležnosti u skladu sa pravilnikom sistema. Vrste odluka, pragovi za donošenje odluka, kvorum i postupak glasanja utvrđuju se Pravilnikom o Gornjem Kolu. U pogledu raspoređivanja dinarskih sredstava, Gornje Kolo upućuje preporuke Upravnom odboru fondacije, koji ih razmatra i primenjuje u okviru svojih zakonskih ovlašćenja po Zakonu o zadužbinama i fondacijama — sa obavezom obrazloženog odgovora na svaku preporuku. Fondacija u ovoj fazi zadržava servisnu ulogu — drži infrastrukturu, zastupa sistem u pravnom prometu i sprovodi odluke Gornjeg Kola. Gornje Kolo je telo obrazovano pravilnikom fondacije i ne ubraja se u njene organe utvrđene statutom; odluku Gornjeg Kola sprovodi Upravni odbor svojim aktom i dužan je da ga donese, a odbiti može samo ako je odluka suprotna zakonu ili statutu, ako zadire u pitanja izuzeta iz nadležnosti Gornjeg Kola ili ako je na snazi zaštitni veto — uvek uz obrazloženje koje se objavljuje. Time nadležnost ostaje tamo gde je zakon postavlja, a fondacija se sopstvenim aktom obavezuje da po odluci postupi. Fondacija zadržava i zaštitni veto na odluke Gornjeg Kola. Veto se gasi trajno i jednosmerno kada finansijska sredstva Fondacije dostignu prag finansijske samostalnosti utvrđen posebnim pravilnikom.
...
Verifikator je korisnik koji potvrđuje stvarnost drugog korisnika na osnovu ličnog poznavanja (poglavlje 7). Verifikator ima dva podsticaja.
...
Nosilac ZRNA ima i mogućnost otpisa — vraćanja slobodnog ZRNA u fond raspoloživih uz evidenciju POEN-a po tekućem obračunskom koeficijentu (odeljak 6.2). Ta mogućnost nije podsticaj: razlika prema broju POEN-a utrošenom pri upisu nastaje u oba smera, nijedno lice je ne obećava, a sopstveni upis i otpis koeficijent ne pomeraju (odeljak 6.4). Eventualna korist je uz to ograničena na interni kapacitet razmene, jer POEN-i ne mogu da napuste sistem. Istovremeno, strukturni izbor između upravljačke funkcije (aktivno ZRNO) i obračunske fleksibilnosti (slobodno ZRNO) sprečava istovremenu realizaciju obe koristi.
...
Aktivira se dokaz stvarnosti (poglavlje 7). Prva grupa korisnika prolazi verifikaciju kroz lanac potvrda. Članovi Upravnog odbora fondacije kao polazni korisnici obezbeđuju verifikacioni kapacitet za pokretanje lanca potvrda.
...
**Neverifikovani korisnik — **Lice registrovano na platformi čija stvarnost nije potvrđena kroz lanac potvrda. Može da pregleda sistem, da razmenjuje dobra i usluge van prostora za oglašavanje i da učestvuje u ažuriranju evidencije POEN-a (kao davalac ili primalac), i priprema se za verifikaciju. Nema pristup evidentiranju doprinosa (emisiji POEN-a kroz kanale), doniranju, postavljanju oglasa ni upravljanju. Ulazni status. Poglavlje 7.

**Verifikovani korisnik — **Lice čija je stvarnost potvrđena kroz lanac potvrda i čiji je indeks stvarnosti najmanje 10%. Razmenjuje, doprinosi, stiče evidenciju POEN-a, donira, učestvuje u krugovima i zadrugama. Potpun i legitiman status. Poglavlje 7.
...
**Lanac potvrda — **Mehanizam dokaza stvarnosti u kome postojeći verifikovani korisnici potvrđuju stvarnost novih korisnika na osnovu neposrednog poznavanja. Poglavlje 7.
...
**Polazni mehanizam (bootstrap) — **Mehanizam pokretanja lanca potvrda u kome članovi Upravnog odbora fondacije dobijaju početni indeks bez verifikacije od strane drugih korisnika. Poglavlje 7.
...
**Socijalni programi — **Mehanizam automatskog evidentiranja POEN-a za kvalifikovane grupe korisnika čije strukturno učešće u zajedničkom dobru protokol prepoznaje iako se ne ispoljava kroz pojedinačne aktivnosti. Početne grupe: roditelji, stariji korisnici, osobe sa invaliditetom, učenici i studenti. Poglavlje 9, Modul 3.
...
*KOLO: *Sistem primenjuje graduirane sankcije za kršenja — posebno za lažnu verifikaciju: zabrana vršenja daljih verifikacija, oduzimanje prava na ZRNO, ukidanje naloga (poglavlje 7). Sankcije su proporcionalne — trošak lažne verifikacije raste sa pozicijom verifikatora u sistemu. Verifikator koji lažno potvrdi rizikuje celokupnu akumuliranu evidenciju POEN-a i evidentiran položaj (poglavlje 11). Nekonvertibilnost obezbeđuje da je interna pozicija jedina stvar koju korisnik može da izgubi — ali za aktivnog korisnika to je značajan gubitak.
