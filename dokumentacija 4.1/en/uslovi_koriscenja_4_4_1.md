...
A user may report an individual message in the Chat room. A report is a signal to the Foundation and does not itself remove the message; removal is decided by the Foundation under the rules of this Article. A single user may submit at most one report per message. In the children's Chat room the report is the only path to the Foundation, as the parent does not read conversations between minor users (Rulebook on the Participation of Children).
