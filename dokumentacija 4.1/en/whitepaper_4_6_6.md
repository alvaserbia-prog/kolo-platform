...
The integrity of the system rests on a proof-of-reality model — a chain of confirmations based on personal acquaintance — in which existing participants confirm the reality, uniqueness, and continuity of new users. This model is a deliberate design decision that minimises the collection of personal data, in keeping with the data-minimisation principle built into the system.
...
ZRNO cannot be transferred between users. There is no mechanism — either in the Protocol or outside it — by which a user could transfer their ZRNO record to another user. This is not a technical limitation that could be circumvented — it is a design rule of the system. ZRNO is a non-transferable record tied to the identity of the user confirmed through the chain of confirmations — even if a user were to attempt to assign access to an account, the ZRNO remains tied to the physical person whose reality was confirmed by verifiers, thereby preventing functional transfer. The non-transferability of ZRNO means that there is no market for ZRNO, no price for ZRNO, and no possibility of speculating with ZRNO.
...
The redemption mechanics. A user who inscribes ZRNO at one accounting coefficient and redeems it at another receives a different number of POEN than the one used as the basis for inscription — in both directions, because the coefficient rises with the inscription of new POEN records and falls with their annulment (Section 6.4). Four elements break the classification of that difference as a return: POEN recorded through redemption have no external financial value — they cannot be converted into money, taken out of the system, or monetised (the principle of non-convertibility); a change of the coefficient is not guaranteed — it arises from the state of the ledger of the entire community, not from the efforts of third parties within the meaning of an investment contract; one's own inscription and redemption do not shift the coefficient at all, so the Holder has neither an instrument nor a moment by which to realise the difference; there is no issuer who promises the growth of the coefficient nor any institution that pays the difference. Additionally, the system's structure establishes a structural choice that limits purely passive holding: a holder who wants the governance benefit must activate ZRNO, thereby losing the ability to redeem; a holder who wants accounting flexibility cannot simultaneously vote.
...
The coefficient is shifted exclusively by changes in the total number of recorded POEN — upward and downward.
...
When recorded POEN are annulled — upon termination of user status, upon annulment by utilisation in a collective procurement, upon write-off for a terminated friendship in the children's space, or on another ground laid down by a bylaw — the numerator decreases. This changes the accounting coefficient downward. The coefficient is therefore not monotonic and is not predetermined to rise.

The exchange of goods and services does not affect the coefficient because it redistributes existing POEN without changing the total number.

The inscription and write-off of ZRNO likewise do not shift the coefficient, and this follows from the formula itself rather than from a rule that could be amended. Upon inscription the denominator decreases by the ZRNO inscribed, but the numerator simultaneously decreases by the POEN returned to the Protocol by that inscription and withdrawn from circulation — and that number is precisely the product of the ZRNO inscribed and the prevailing coefficient, so the quotient remains the same. Upon write-off both terms grow in the same ratio. A ZRNO Holder cannot, by their own conduct, shift their own coefficient in either direction.

All these effects are consequences of user activity in the system. No individual user controls the coefficient. No institution controls the coefficient. The coefficient is a calculated value arising from the total state of the ledger of all users in the system.
...
A user who contributes to the system in the early phase — when the coefficient is low — inscribes ZRNO with fewer recorded POEN than a user who does the same in a later phase with a higher coefficient. The position of an early participant thereby reflects their contribution in the phase when contribution was most valuable for the establishment of the system. That is a consequence of recognising an earlier contribution to the common good, not an incentive to acquire a position: the number of POEN required for inscription is calculated by the Protocol from the state of the ledger, not by a decision that would reward early participation, and no person promises that this number will later be higher.

The rule of one percent of balance per accounting period (Section 6.2) limits the rate at which available ZRNO is taken up — even when a large number of users simultaneously meets the conditions for inscription, the total volume of inscription per period is limited to one percent of the total balance of all qualifying users. This preserves access for later participants and keeps the coefficient less sensitive to individual inscriptions of new POEN records, because the number of available ZRNO is not exhausted abruptly.
...
Every user of the KOLO system must confirm their reality, uniqueness, and continuity through a verification model based on personal acquaintance — a chain of confirmations in which existing participants confirm new ones. The model does not require the collection of personal identity documents. Each user has a reality index (0–100%) that determines the scope of access to system functions and their verification capacity.
...
Separate identification mechanisms apply to the monetary flow: donor verification is provided through the banking system — the Foundation receives donations from verified bank accounts; sponsor verification (legal entities and entrepreneurs) is provided on the basis of a donation agreement with the Foundation. These mechanisms relate to identification for the purposes of the Foundation's financial flow, not to proof of reality of users in the sense of the chain of confirmations.

## Unverified User

An unverified user is a person registered on the platform whose reality has not yet been confirmed through the chain of confirmations. They may browse the system and familiarize themselves with the rules, but have no access to exchange, POEN ledger recording, or donation.

The transition to verified user status occurs when an existing verified user confirms the reality, uniqueness, and continuity of a new user through the chain of confirmations, whereby the user acquires a reality index of at least 10% and full access to the system.

## Verified User

A verified user is a person whose reality has been confirmed through the chain of confirmations and whose reality index is at least 10%. A verified user exchanges goods and services with other users within the system. They contribute to the common good through activities whose contribution is recorded in POENs. They may donate funds in dinars to the Foundation. They may participate in Circles and Cooperatives when those modules are activated (Chapter 9).
...
The Protocol does not record the founding contribution all at once, but ties it to system growth — recording it in fixed-amount steps, according to the cumulative growth of the total number of POENs in the system. This is a design decision. Each inscription of new POEN records shifts the accounting coefficient (Section 6.3); recording the full amount at once would produce a sudden jump in the coefficient, while tying it to system growth means the coefficient reaches the same level in proportion to that growth. The channel follows growth whatever its pace may be: if the total number of POENs grows abruptly, the steps are recorded at the same speed at which the growth occurred.

Since the step is fixed, the impact of a single step on the coefficient decreases as the system grows. The share of the aggregate recorded founding contribution in the total number of POENs is a different quantity, and it does not decrease: both the thresholds and the step are expressed in fixed amounts, and POENs recorded through this channel enter the total number, so the aggregate founding contribution constitutes between approximately one fifth and approximately one quarter of the total number of recorded POENs throughout the entire life of the channel — at closing, approximately 24%. That share carries no property right and constitutes no claim; the circle of founders is closed and cannot be expanded, and upon reaching the upper limit the channel closes permanently.

The upper limit is a measure of the contribution that preceded the system: the founders invested their own monetary resources and their own time in the design, construction, and legal and organisational preparation, in a period in which no recording channel was in operation. That work cannot be recorded through operational contribution even retrospectively — the daily limit of that channel amounts to 10% of the total number of recorded POENs, so while that number is zero, the limit is zero as well.

The result of founding work — the software, the Protocol, and the documentation of the system — enters the common good under the licences set out in Chapter 3: software under AGPL-3.0, content under CC BY-SA 4.0. Founders acquire no exclusive right to that result on the basis of that work; it is available to all persons on the same terms, and the licences cannot be replaced with more restrictive ones.
...
The system applies a limit of 10% of the total number of POENs recorded in the system per accounting period to the sum that may be recorded in that period through operational contribution and support programmes together. That limit is not an upper bound on the total number of POENs but the key by which the period's issuance is divided when the sum of requested amounts exceeds the limit: all amounts are then reduced proportionally, and the unrecorded surplus is not carried over to the next period. The total number of POENs is not capped by any predetermined figure — it follows the number of participants and their involvement, because every entry is tied to the contribution of a specific person, and it is not one-directional, because annulments of entries reduce it. This parameter is subject to change through the governance processes described in Chapter 10. The procedure of application, execution, and verification is defined in the Rulebook.
...
Automatic recording in social programmes is not remuneration — it pays for no work, service or other performance, nor is anything required of the user in return towards the foundation. The programmes are carried out in pursuit of the objectives of the foundation laid down in Article 6 of the Statute, in particular social protection and solidarity support for vulnerable social groups, and they enable users to participate in the system on a more equal footing.
...
A verifier is a user who confirms the reality of another user on the basis of personal acquaintance (chapter 7). The verifier has two incentives.
...
The verifier also has a structural constraint — they put their own position in the system at stake as a guarantee of the accuracy of the verification. Graduated sanctions for false verification — a ban on performing further verifications, revocation of the right to ZRNO, account cancellation — ensure that the cost of false verification is proportionate to the benefit of it. A verifier who confirms falsely risks their own accumulated POEN records and their recorded position in the system. This structure means that the rational choice for a verifier is to confirm only persons whose reality they actually know — the benefit from a single false verification (POENs for verification) is disproportionately smaller than the potential loss (the entire position in the system).
...
A ZRNO Holder also has the option of redemption — returning free ZRNO to the pool of available units, with the Protocol recording POEN at the current accounting coefficient (section 6.2). That option is not an incentive: the difference against the number of POEN used upon inscription arises in both directions, no person promises it, and one's own inscription and redemption do not shift the coefficient (section 6.4). Any benefit is moreover limited to the internal exchange capacity, because POEN cannot leave the system. At the same time, the structural choice between the governance function (active ZRNO) and accounting flexibility (free ZRNO) prevents simultaneous realization of both benefits.
...
Proof of reality is activated (chapter 7). The first group of users passes verification through the chain of confirmations. Members of the Foundation's Board of Directors as the initial users provide the verification capacity for starting the chain of confirmations.
...
Operational contribution is activated (chapter 8). Users apply for tasks for the common good by submitting an execution plan. Members of the Foundation's Board of Directors approve plans and verify daily execution. Upon activation of the Upper Kolo in Phase 2, this function is taken over by ZRNO Holders. The limit of 10% of the total number of POENs recorded in the system per accounting period divides that period's issuance between operational contribution and support programmes when the sum of requested amounts exceeds the limit; it does not set an upper bound on the total number of POENs. This limit is an operational parameter subject to change through the governance processes (chapter 10).
...
**Unverified user —** A person registered on the platform whose reality has not been confirmed through the chain of confirmations. Can view the system, exchange goods and services outside the advertising space, and participate in POEN ledger updates (as giver or receiver), and prepare for verification. Has no access to contribution recording (POEN issuance through channels), donating, posting listings, or governance. Entry-level status. Chapter 7.

**Verified user —** A person whose reality has been confirmed through the chain of confirmations and whose reality index is at least 10%. Exchanges, contributes, acquires POEN records, donates, participates in Circles and Cooperatives. Full and legitimate status. Chapter 7.
...
**Chain of confirmations —** The proof-of-reality mechanism in which existing Verified users confirm the reality of new users based on direct personal acquaintance. Chapter 7.
...
**Bootstrap mechanism —** The mechanism for starting the chain of confirmations, whereby members of the Foundation's Board of Directors receive an initial index without verification by other users. Chapter 7.
