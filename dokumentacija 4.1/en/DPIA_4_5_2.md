...
| **Related documents** | Rulebook on the KOLO System (v4.5.2), Privacy Policy (v4.5.2), Records of Processing Activities (v4.5.2), Rulebook on Support Programmes (v4.5.0), Rulebook on the Hierarchy of Acts (v4.4.6), Statute (v4.1), Whitepaper (v4.4.6) |
...
Detailed categories of data, categories of data subjects, recipients, retention periods, and protection measures for each processing activity are established by the Records of Processing Activities (v4.5.2) applied together with this assessment.
...
POEN issuance, calculation of the accounting coefficient, and the annulment of a confirmation of reality due to inactivity in the procedure under Article 6 of the Rulebook on the Participation of Children are automated processing activities that may have a legal or significant effect on a person within the meaning of Art. 38 LPDP. The annulment is the only one of them that touches a person's status: it takes effect upon expiry of the period, without a decision of an organ of the Foundation, and its consequence is the loss of the confirmation given and the write-off of the POEN recorded on account of it, even where the record thereby becomes negative. Special guarantees therefore accompany it: notification of every person who would be affected by the annulment before the period expires, an express right of appeal decided upon by a human, and a record that does not label the annulment as an established untrue confirmation. The Foundation provides: a deterministically defined public formula for the accounting coefficient, the user's right to an explanation of the logic, the right to human review, and the right to object. The public nature of the formula and the deterministic approach reduce the risk of arbitrariness. Conclusion: the processing is necessary for the functioning of the system, with adequate guarantees.
...
| R2 — Re-identification of pseudonymized data | 2 | 3 | 6 | Combining the pseudonymous ledger with external sources may lead to identification. In addition, upon termination of status the account is pseudonymized, not anonymized: the Foundation retains the internal identifier, so the retained records remain personal data and re-identification is possible on its side. Measures: no centralized table linking a pseudonym to a name, separation of identification and accounting data, restricted access, the measures in section 5.14. |
...
**Confirmation that the child exists requires a declaration from both parties** — an account opened under Article 4, or taken over under Article 4b, is accompanied by the parent's statement under full responsibility, the text of which is recorded and is not altered after it is given; the persons who have confirmed the parent's reality make their declaration within sixty days, and the confirmation survives only if both parties to the link make a declaration. **Notification of every affected person before the period expires** — the annulment reduces the record of the person who gave the confirmation, of the parent, and of the supervisor alike, so all of them are notified before the period expires, together with the amount that would be written off. **Annulment is not a qualification** — the record of the affected person states that the confirmation fell because of inactivity in the procedure, not that an untrue confirmation was established; the burden is not transferred to another person, and an appeal decided upon by a human lies against the annulment.
...
## 5.14. Measures for Termination of Status and the Right to Erasure

An accurately named procedure — upon termination of status the account is pseudonymized, not anonymized; the retained records remain personal data, and the limitation on erasure rests on Art. 30 para. 3 LPDP (statutory retention obligation and legal claim), not on a claim that the data have ceased to be personal. Deletion of content — the email address, voluntarily entered data, the content and photographs of listings (the listings are removed from the advertising space, the photographs from storage), and the free text entered alongside re-registrations of POEN are deleted. Deletion of messages — messages in a conversation are deleted when at least one party closes their account. An explicit list of what remains — the numerical records alongside a pseudonym that does not point to a name, the basis on which the POEN was recorded, and the name alongside a public donation; the scope is enumerated in Art. 11 of the Policy instead of being left implied. A warning before the data comes into being — before every public donation the user is informed that the name remains in the list even after the account is closed and that the alternative is an anonymous donation. The right to rectification — a request to rectify inaccurate data is submitted within the Platform, as an appeal of the data-correction type. Verification of retention periods — the Foundation verifies at least once a year whether the retention period has expired for any category of data.
