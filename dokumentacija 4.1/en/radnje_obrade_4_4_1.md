...
*These Records are adopted on the basis of Art. 47 of the Law on Personal Data Protection ("Official Gazette of the RS", no. 87/2018, hereinafter: LPDP), Art. 62 and 63 of the Rulebook on the KOLO System (version 4.4.1), and Art. 9 of the Rulebook on the Hierarchy of Acts of the KOLO System. They are applied together with the Privacy Policy of the KOLO Platform (version 4.4.1) and the Rulebook on Support Programmes (version 4.4.1).*
...
| **Categories of data** | The minor user's pseudonym; the parent's email address entered by the minor; the minor user's own email address, if they state one; the date of birth entered by the parent; the link to one or two parents and their consent; the friendship graph with dates; messages in the children's Chat room and in conversations; POEN records; listings; reports of messages; the school stated by the minor user themselves. |
| **Legal basis** | Consent of the parent or legal guardian (Art. 16 LPDP), with additional restrictions for persons under fifteen years of age. For the parent's email address — legitimate interest (Art. 12(1)(6) LPDP) for the purpose of obtaining that consent. The minor user's own email address is processed under the same consent, its statement being voluntary and its purpose limited to regaining access to the account. |
...
| **Transfer to a third country** | Yes — infrastructure processors are located in the USA (see Art. 9 of the Privacy Policy). |
| **Retention period** | While the account remains active. An account not taken over by a parent within fourteen days of opening is deleted, together with the parent's address and the invitation. Friendships are deleted upon attaining the age of majority and upon deletion of the account. A report of a message — until the procedure is concluded, for no longer than 12 months. The minor user's own email address — until they remove it or until the capacity of user ceases; an unconfirmed address loses effect after twenty-four hours. |
| **Protection measures** | See point 5.11 of the DPIA. Most importantly: only two data items are processed in the window before consent; the message to the parent states only the pseudonym, never the name; two actions to stop the processing without login; full access only once the parent has been confirmed by a third party in the chain of confirmations; the parent does not read conversations between minor users; the adult party is notified that the conversation is read by a parent; the children's Chat room is filtered by the friendship graph, without replies by quotation; reporting a message as the minor user's own path to the Foundation; the profile of a minor user is not available to adult users, a notice with the parent's pseudonym being displayed instead; the overview by school is aggregate and contains no personal data; no history of earlier school choices is kept; the minor user's own email address is recorded only upon confirmation by opening a link sent to that address, no notifications are sent to it, and where there is none, the parent sets the new password. |
