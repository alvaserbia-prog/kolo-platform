...
| **Status** | ACTIVE — Module 3 is activated in accordance with Art. 57 of the Rulebook and the Rulebook on Support Programmes (v4.5.0); activation is accompanied by an updated DPIA (v4.3.0). |
| --- | --- |
...
| **Categories of data subjects** | Users belonging to qualifying groups (parents, elderly persons, persons with disabilities, pupils and students) and their verifiers. |
...
| **Legal basis** | Explicit consent of the user (Art. 17 para. 2 item 1 LPDP), given separately for the application and for requesting confirmation from verifiers. The consent expressly states how many persons will be asked to confirm and that the record of recorded POEN, with the name of the program, is visible to all verified users alongside the pseudonym. Consent may be withdrawn at any time, by an action available to the user alongside the program itself, with the consequence of ceasing the procedure and automatic recording. |
| **Recipients / processors** | Vercel Inc. (hosting, executed in the European Union) and Neon Inc. (database, region in the European Union), on the basis of a processing agreement. The person at the Foundation processing the application has access to the entered data. The applicant's verifiers receive exclusively the request for confirmation (program name and the pseudonym of the applicant whom they personally know) — without access to the entered data. All verified users — through the public ledger of the Protocol they see the record of recorded POEN with the name of the program alongside the user's pseudonym (Art. 6 of the Privacy Policy); they do not see the entered data. |
| **Transfer to a third country** | No for the data in the database — the application and the database are executed in the European Union (Frankfurt region), which provides an adequate level of protection. Vercel Inc. and Neon Inc. are US companies, so access from a third country is possible for administrative and technical purposes, subject to the measures under Art. 9 of the Privacy Policy. |
