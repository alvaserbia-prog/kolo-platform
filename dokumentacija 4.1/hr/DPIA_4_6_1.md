...
| R11 — Otkrivanje pripadnosti posebnoj kategoriji verifikatorima | 2 | 3 | 6 | U postupku potvrde socijalnog programa verifikatori podnositelja saznaju da se prijavio za određeni program, što može ukazati na posebnu kategoriju (npr. invaliditet, status roditelja). Ovom se procjenom vjerojatnost spušta s 3 na 2: pojedinačno evidentiranje po programu izašlo je iz javnog pregleda evidencije, a opis zapisa više ne imenuje program, pa krug primatelja ponovno jest ograničen na vlastite verifikatore podnositelja — osobe koje ga i inače osobno poznaju. Ranija ocjena od 3 počivala je upravo na tome što taj krug nije bio ograničen. Izostavlja se i sam iznos, jer se izvodi iz podataka na temelju kojih je pravo utvrđeno (kod potpore starijima jednoznačno iz godine rođenja, kod potpore majkama iz broja i dobi djece) i odao bi ih i bez naziva. Težina ostaje 3 — riječ je o posebnoj kategoriji. Mjere: vidi odjeljak 5.6. |
...
Izostavljanje pojedinačnog zapisa iz javnog pregleda — pojedinačno evidentiranje doprinosa po socijalnom programu ne prikazuje se u pregledu evidencije i opis zapisa ne imenuje program. Umjesto pojedinačnih zapisa objavljuje se dnevni zbroj po programu (broj korisnika i ukupan iznos), a dan u kojem je u jednom programu doprinos evidentiran samo jednom korisniku ne objavljuje se, jer bi zbroj tada bio jednak pojedinačnom iznosu. Izostavlja se i sam iznos, ne samo naziv: iznos se izvodi iz podataka na temelju kojih je pravo utvrđeno — kod potpore starijima jednoznačno iz godine rođenja korisnika, kod potpore majkama iz broja djece i njihove dobi — pa bi objavljen pojedinačan iznos odao posebnu kategoriju i bez ijedne riječi uz njega. Provjerljivost evidencije time nije umanjena: zbroj objavljenih dnevnih iznosa zajedno s ostalim kanalima daje promjenu ukupnog broja evidentiranih POEN-a, koji je javan. Raščlamba po programu dostupna je samom korisniku, uz program, i Zakladi.
