#!/usr/bin/env bash
# Čišćenje naracije (video prvi oglas): jedan snimak sa telefona -> audio/clean/glas.wav
...
ffmpeg -v error -y -i audio/raw/snimak53.m4a -af "highpass=f=70:poles=2,volume=-5dB" -ar 48000 -ac 1 -c:a pcm_f32le "$T/in/glas.wav"
