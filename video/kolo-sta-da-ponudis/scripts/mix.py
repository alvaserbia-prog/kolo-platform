"""Miks (video 5 „Šta da ponudiš“): devet klipova naracije na mestima iz plana + muzika sa duckingom, -14 LUFS.

Glas: iz audio/final/glas.wav seče se [klipOd, klipDo] svake scene i postavlja na glasOd.
Muzika: audio/muzika-slozena.wav (numera iz videa 3, produžena celim taktovima), -8 dB, rez na 2,6 kHz, sidechain vođen glasom.
Efekti: audio/sfx.wav (scripts/sfx.py), −3 dB, bez duckinga.
...
n = len(plan["scene"])
ulazi += ["-i", "audio/muzika-slozena.wav", "-i", "audio/sfx.wav"]
...
filt.append(f"[{n+1}:a]aformat=channel_layouts=stereo,aresample=48000,volume=-3dB,apad=whole_dur={T},atrim=0:{T}[sfx]")
