"""Zbijanje pauza + blago ubrzanje naracije (video 5, jedan snimak).
...
PRAG_SCENA = 1.0
TEMPO = 1.04
# granice scena u sirovom snimku (s): posle njih ostaje duža pauza bez obzira na dužinu
GRANICE = [6.30, 11.54, 19.48, 27.93, 33.17, 41.48, 47.09, 60.97]
PRAG_DB = 34
KRAJ_SNIMKA = 71.0
...
while k < zadnji:
    if tiho[k]:
        ...
        while j < zadnji and tiho[j]:
            j += 1
        duz = (j - k) / 100
        scena = duz > PRAG_SCENA or any(abs(k/100 - g) < 0.05 for g in GRANICE)
        cilj = PAUZA_SCENA if scena else PAUZA_FRAZA
        if duz > cilj:
            ostavi = int(cilj * 100)
            sredina = k + ostavi // 2
