...
- **Feed prepoznavanja** (`GET /api/tabla-jemstva/feed`, samo verifikovani): jedna kartica, jedno pitanje „Poznaješ li ovu osobu?" → `POST /[id]/prepoznajem {odgovor: DA|NE|NISAM_SIGURAN}` → model `Prepoznavanje` (`@@unique([zahtevId, korisnikId])`). **Sva tri odgovora se upisuju** — bez toga feed vrti iste ljude u krug. Rangiranje: ista **opština** (ne tačno naselje — `NASELJE_OPSTINA`, jer čovek iz sela pored Sombora prepoznaje Somborca) → +isto naselje → +blisko godište (≤7 g.). Godište posmatrača se čita iz njegove sopstvene nekadašnje kartice (nema polja na nalogu). Isključeni: sopstveni zahtev, već odgovoreni, nepotpune kartice, osnivači, zabranjena zona (postojeći keš `verifikacionaZona`, oba smera).
...
- **Legacy:** stari slobodan tekst se **ne parsira automatski** (pogrešno izvučeno ime je gore od nikakvog) — arhivira se u `legacyTekst`, status → **`NEPOTPUN`**. Nepotpune kartice stoje na zidu, ne ulaze u feed i **ne mogu se verifikovati** dok ih vlasnik ne dopuni (`PATCH /api/tabla-jemstva`, ne produžava rok).
