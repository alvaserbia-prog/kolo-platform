...
### Tabla zahteva za jemstvo — kartica prepoznavanja (redizajn 2026-08-03)
- Neverifikovan korisnik se predstavlja mreži radi verifikacije (Pravilnik čl. 32, Uslovi).
- **Objava se više NE PIŠE kao slobodan tekst, nego se SASTAVLJA iz polja.** Povod: slobodno polje su ljudi popunjavali kao oglas („bavim se pčelarstvom") — formalno validno, a niko ne može da prepozna osobu. Polja kartice: `ime`, `prezime`, `godiste`, `mesto` (iz šifarnika `NASELJA_SRBIJE`), `nadimak`, `cimeSeBavi` (max **100** znakova). **Sva su OPCIONA** — nikoga ne zaustavljamo na vratima; ali kartica bez `ime`+`mesto` **ne ulazi u feed** (bez mesta se ne može nikome dovesti), samo stoji na zidu uz poruku vlasniku šta da doda.
- **Skriveni kontakt (nikad na kartici):** `telefon` + `telefonSaglasnost` — vidi ga **samo UO**, preko `GET /api/admin/tabla-jemstva/neprepoznati` (spisak ljudi koje niko nije prepoznao; svaki pristup se loguje `PRISTUP_NEPREPOZNATI_JEMSTVO`). Briše se pri verifikaciji, isteku i uklanjanju (minimizacija). **Ranije polje `kontaktPodaci` i ruta `/[id]/kontakt` su UKLONJENI** iz forme, prikaza i API-ja; kolona ostaje u bazi jedan ciklus radi povratka. **Link ka profilu (FB/IG) se NE prikuplja** — kratko je postojao kao `linkProfila`, pa uklonjen odlukom vlasnika (migracija `20260803140000_bez_linka_profila`); po istoj logici po kojoj nema fotografije lica.
- **Feed prepoznavanja** (`GET /api/tabla-jemstva/feed`, samo verifikovani): jedna kartica, jedno pitanje „Poznaješ li ovu osobu?" → `POST /[id]/prepoznajem {odgovor: DA|NE|NISAM_SIGURAN}` → model `Prepoznavanje` (`@@unique([zahtevId, korisnikId])`). **Sva tri odgovora se upisuju** — bez toga feed vrti iste ljude u krug. Rangiranje: ista **opština** (ne tačno naselje — `NASELJE_OPSTINA`, jer čovek iz sela pored Sombora prepoznaje Somborca) → +isto naselje → +blisko godište (≤7 g.). Godište posmatrača se čita iz njegove sopstvene nekadašnje kartice (nema polja na nalogu). Isključeni: sopstveni zahtev, već odgovoreni, osnivači, zabranjena zona (postojeći keš `verifikacionaZona`, oba smera). **Prazna polja nikoga ne izbacuju iz feed-a** — samo ga rangiraju na kraj.
- **⚠️ Prepoznavanje NIJE jemstvo.** Odgovor „DA" otvara SAMO `Konverzacija`. Formalni čin jemstva je zaseban akt sa zasebnom potvrdom (`/[id]/verifikuj`) — ne spajati ni u UI ni u bazi.
- **Vidljivost:** verifikovan → puna kartica; neverifikovan → **inicijali, mesto, decenija rođenja** (`karticaZaPosmatraca`). Tabla ostaje iza prijave, `robots: noindex` (NIJE otvorena neregistrovanima).
- **🔴 POPUNJENOST KARTICE NIJE USLOV ZA VERIFIKACIJU.** Verifikuje se čovek, ne kartica — ko nekoga stvarno poznaje sme da mu jemči i sa poluprazne kartice. Prazna polja utiču **samo na redosled u feed-u** (kartica bez mesta ide na kraj reda, ali ide). Ovo je bila regresija u prvoj verziji redizajna: legacy objave su prebačene u `NEPOTPUN`, a `izvrsiVerifikacijuSaTable` je primao samo `AKTIVAN` → te ljude niko nije mogao da verifikuje (410). Ispravljeno migracijom `20260803160000_bez_nepotpune_kartice` (NEPOTPUN → AKTIVAN); **status `NEPOTPUN` se više NE upisuje** (enum vrednost ostaje jer PostgreSQL ne dozvoljava brisanje bez rekreiranja tipa). Ne vraćati nikakav uslov popunjenosti.
- **Legacy:** stari slobodan tekst se **ne parsira automatski** (pogrešno izvučeno ime je gore od nikakvog) — arhivira se u `legacyTekst` i prikazuje na kartici. Vlasnik dobija **savet** da dopuni karticu (`PATCH /api/tabla-jemstva`, ne produžava rok) — savet, ne uslov.
- **🟡 Rok je produžen sa 72h na 8 dana** (`TRAJANJE_DANA` u `src/lib/jemstvo-kartica.ts`) jer je struktuirana kartica skuplja za popunjavanje. **Politika 3.9.1 i Uslovi čl. 16 još uvek navode 72h — akte treba uskladiti** (svesno odloženo, odluka vlasnika). Nova polja su lični podaci → Registar radnji obrade takođe treba dopuniti.
- Cron ostaje **dnevni** `0 4 * * *` — GET filtrira `expiresAt>now` u realnom vremenu, pa subdnevni schedule NE treba; subdnevni cron Vercel plan ODBIJA i obara build sa cron-pricing greškom.
- **Verifikacija sa table:** verifikator (indeks ≥10%/NOSILAC_ZRNA) klikne „Verifikujem" na kartici → `POST /api/tabla-jemstva/[id]/verifikuj` → `izvrsiVerifikacijuSaTable` (deli jezgro sa token putem); objava = pristanak, bez trećeg koraka; verifikovani dobije notifikaciju + „Prijavi" (`/api/verifikacija/prijavi` → RizikNalaz).
- **Izolovani korisnik:** ako ga niko ne prepozna a dao je saglasnost, član UO ga pozove i — ako oceni — verifikuje ga **U SVOJE IME**, kao član u lancu jemstva. Fondacija time NE postaje verifikator (nema izmene u logici verifikacije). Posledica: svi koje isti član verifikuje postaju međusobno „braća" po čl. 12 i ne mogu jedan drugog verifikovati.
- **Kod:** `src/lib/jemstvo-kartica.ts` (čiste funkcije, testovi `__tests__/jemstvo-kartica.test.ts`), komponente `src/components/jemstvo/` (`KarticaForma`, `KarticaPrikaz`, `FeedPrepoznavanja`, `NeprepoznatiSpisak`). **Forma postoji na JEDNOM mestu** — `KarticaForma` je dele zid i `/verifikacija` (ranije prepisana u dva fajla). Migracije `20260803120000_kartica_prepoznavanja` (aditivna) + `20260803120100_kartica_backfill_legacy` (zaseban fajl jer PostgreSQL ne dozvoljava upotrebu nove enum vrednosti u transakciji u kojoj je dodata) + `20260803140000_bez_linka_profila` + `20260803160000_bez_nepotpune_kartice`. **Primenjenu migraciju NIKAD ne menjati** — Prisma je proverava kontrolnom sumom i `migrate deploy` puca; ispravka ide kao nova migracija.
