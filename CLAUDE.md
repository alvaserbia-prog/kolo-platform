...
**AŽURIRANO 2026-08-09 (treći put):** uz set 4.1.1, **četiri akta su dignuta na 4.2.0** — `Pravilnik_4_2_0.md`, `dokaz_stvarnosti_4_2_0.md`, `DPIA_4_2_0.md` i `radnje_obrade_4_2_0.md` (sr + en/ru/hr/hu). Povod je paket „nadzor verifikacija dobija glas + lažnost se ceni po čoveku" — vidi zasebnu sekciju u „Ključni koncepti". Fajlovi žive u **istom folderu `dokumentacija 4.1/`** (isti postupak kao raniji bump `dokaz_stvarnosti_4_0_1.md`); loader, mapa u `pravilnik/[slug]`, stranice `/dpia` i `/radnje-obrade` i verzijske labele u `messages` su repointovani. Ostalih 11 akata ostaje **4.1.1**. Labela na indeksu `/pravilnik` (`pravne.pravilnik.ver`) prati najnoviji akt, pa piše 4.2.0; Uslovi i Politika ostaju 4.1.1.

🔴 **Set je od sada MEŠOVIT po verzijama** — ne postoji jedan broj koji važi za ceo folder. Ko dalje diže verziju mora da gleda verziju **po aktu**. Konkretna zamka koja se već desila: 4.2.0 dokumenta su nastala iz 4.1.0 osnove dok je `main` u međuvremenu izdao 4.1.1, pa bi objava tiho poništila izmenu čl. 40a. Zato su regenerisana iz 4.1.1 osnove, a test `pravni-dokumenti.test.ts` sada izričito traži rečenicu „Verifikovanom korisniku doprinos se evidentira" u `Pravilnik_4_2_0.md` — da se isti propust ne ponovi tiho.
...
- **Izuzetak od temeljne odredbe:** Pravilnik čl. 14 st. 3 („nijedan korisnik ne može imati negativan zapis POEN-a") dobio je izuzetak za nadoknadu — **jedini**. Isto i čl. 34 (nadoknada preživljava prestanak statusa). Zero-sum ostaje netaknut: prenos minusa je običan prenos.
...
- **Prelazna radnja za zatečene (čl. 40a, poslednji stav):** `evidentirajZateceneVerifikovane()` + dugme **Osnivači → „Evidentiraj zatečene"** (`POST /api/admin/doprinos-sadrzaju/zatecene`, samo superadmin). Razrešava doprinose koji stoje `ZABELEZEN` kod verifikovanih; neverifikovane preskače. Idempotentno. **Namerno na dugmetu, ne u migraciji** — emisija mora kroz `emitujPoen` (zero-sum, audit), a opticaj skače za 1.000 × broj razrešenih, što može da upali osnivački korak; taj trenutak bira čovek. Dugme uz to **naknadno javlja** onima kojima je doprinos evidentiran a obaveštenje nije otišlo (vidi `obavestenAt` ispod).
...
- 🔴 **Brane protiv naduvavanja opticaja UKLONJENE (odluka vlasnika, 2026-08-09)** — plan je predviđao dve (ručno okidanje osnivačkog koraka + admin alert na obrazac okidača); obe su uklonjene istog dana kad su i objavljene. Jedina brana koja ostaje je **razdvajanje BELEŽENJA od EVIDENTIRANJA** (čl. 40a st. 3), koje je normativno i ostaje.
...
- **Kod:** `src/lib/doprinos-pravila.ts` (ČISTE funkcije — bez Prisme, jer ih uvozi i forma oglasa u pretraživaču) + `src/lib/protokol/doprinos-sadrzaju.ts` (servisne; re-eksportuje pravila, pa server ima jedan ulaz). Testovi `__tests__/protokol/doprinos-sadrzaju.test.ts`. Migracije `20260809120000_doprinos_sadrzaju` (model + enumi + `TransactionType.EMISIJA_SADRZAJ`) → `20260809120100_tabla_jemstva_podaci` (brisanje redova `Prepoznavanje`/`ZahtevZaJemstvo`; zaseban fajl jer nova enum vrednost ne sme u istu transakciju) → `20260809120200_doprinos_retroaktiva` → `20260809130000_doprinos_objava_verifikovan` (nova vrednost okidača `OBJAVA_VERIFIKOVAN`; opet zaseban fajl — Postgres ne da da se nova enum vrednost koristi u transakciji u kojoj je dodata) → `20260809140000_doprinos_obavesten` (`obavestenAt`).
...
- 🔴 **Korak 1 je ZATEČENI čl. 40a i NIJE diran** — ni iznos, ni odloženo evidentiranje, ni `DoprinosSadrzaju`. Nova tabela `DoprinosRazmeni` nosi **samo korake 2–5**, uz `CHECK (korak BETWEEN 2 AND 5)` i `@@unique([userId, korak])`. **Kapu time drži BAZA, ne kod:** najviše četiri reda × 1.000 + 1.000 iz čl. 40a = 5.000. Ne dodavati proveru kape u kodu — bila bi druga istina.
- **Koraci se otključavaju REDOM** (`dostignutKorak` = najviši korak do koga su svi prethodni ispunjeni). Ko ima deset sagovornika a nije objavio tri oglasa, stoji na koraku 2. Posledica koju treba znati: korak 2 traži upis POEN-a, što neverifikovani ne sme (čl. 28 st. 2) — pa su koraci 2–5 faktički zatvoreni dok se čovek ne verifikuje, iako je mašinerija `ZABELEZEN`/`EVIDENTIRAN` ista kao kod čl. 40a i radi za oba slučaja.
- **Pravila brojača** (`sagovorniciUBrojacu`, tri sita): razmena mora biti **obostrano označena**; sagovornik mora biti **van lanca**; ne sme biti **povratnog toka POEN-a u roku od 60 dana**. Uz to: svaki sagovornik **broji se jednom za celu lestvicu** (otud `Set`, ne broj razmena), a razmena sa neverifikovanim se **beleži pa broji tek po njegovoj verifikaciji**.
- 🔴 **„Van lanca" se utvrđuje U TRENUTKU realizacije i TRAJNO pamti** — kolona `Razmena.vanLanca`, ne izvedena vrednost iz `verification_zone`. Kasniji rast lanca ne poništava izbrojane sagovornike. Ne menjati u „čitaj zonu pri brojanju" — brojač bi se sam praznio kako mreža raste.
- **Definicija „van lanca" = nijedno nije u zoni drugog** (`verifikacionaZona.count` u oba smera) — ista tabela po kojoj se sudi ko koga sme da verifikuje.
- **Povratni tok obara sagovornika, ali NE poništava već dodeljen korak.** Zapis POEN-a koji je nastao ne poništava se unazad — za to postoji propisan postupak (dokaz stvarnosti Glava VIII) i ne radi se usput. Brojač se od tog trenutka računa bez tog sagovornika, pa utiče na **buduće** korake.
...
- **Obostrana potvrda:** `Razmena` (PREDLOZENA → REALIZOVANA/ODBIJENA), po jedan red na par (oglas, sagovornik). **Oglašivač sme da imenuje samo onoga ko mu se javio** (`OglasUpit`); sagovornik imenuje sebe. Odbijanje pogađa samo `PREDLOZENA` — realizovana razmena se ne obara unazad.
- **`OglasUpit`** nastaje kad razgovor krene sa stranice oglasa: `POST /api/poruke` prima **opcion `oglasId`**. Ne čuva sadržaj poruke — za brojač je merodavno samo ko se javio i povodom čega.
- **Okidači preračunavanja:** objava oglasa, upit, prenos POEN-a (obe strane), realizacija razmene (obe strane), i **verifikacija** — koja preko `osveziSagovornike()` pomera i TUĐE brojače (razmena sa neverifikovanim počinje da se broji). Sve `probajNapredovati`/`probajEvidentiratiKorake` idu **VAN `prisma.$transaction()`** i **ne bacaju** — isti obrazac kao čl. 40a.
- **Kod:** `src/lib/doprinos-razmeni-pravila.ts` (ČISTE funkcije — pragovi, sita, redosled; bez Prisme jer ih uvozi i prikaz) + `src/lib/protokol/doprinos-razmeni.ts` (servisne, re-eksportuje pravila). Rute: `GET/POST /api/pijaca/[id]/razmena`, `GET /api/doprinos-razmeni`. UI: `RazmenaBlok` u `OglasDetalj.tsx`, `PutanjaRazmene.tsx` u Novčaniku (Suspense). Migracija `20260809160000_doprinos_razmeni`. Testovi `__tests__/protokol/doprinos-razmeni.test.ts`. Audit: `DOPRINOS_RAZMENI_{ZABELEZEN,EVIDENTIRAN}`. Transakcija: `EMISIJA_RAZMENA` (zaseban tip od `EMISIJA_SADRZAJ`, da se u istoriji vidi šta je došlo sa lestvice).
...
- 🟡 **Korak 5 je za sada nedostižan.** Deset realizovanih razmena sa deset različitih ljudi van lanca, pri desetak transakcija u celom sistemu, znači da će lestvica prvih meseci praktično stajati na koraku 3. To je u redu za vrh putanje; ako treba da radi odmah u beti, spušta se **na jednom mestu** — `PRAG_SAGOVORNIKA_KORAK_5` u `doprinos-razmeni-pravila.ts` (npr. na 7), bez ijedne druge izmene.
- 🔴 **NORMATIVNI GAP — akti nisu dopunjeni.** `Pravilnik_4_2_0.md` čl. 40a i dalje propisuje **jednokratnih 1.000 POEN** za prvi oglas; lestvica od pet koraka, kapa od 5.000, rok od 60 dana i pojam „van lanca" **nisu u aktima**. Kod je time ispred normative. Pre puštanja u rad ide dopuna čl. 40a (i po potrebi Uslova — obostrano označavanje razmene je nova radnja korisnika) i nova radnja obrade u DPIA/Radnjama obrade (`Razmena`, `OglasUpit` su nova obrada podataka o ponašanju).
