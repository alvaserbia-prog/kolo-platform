...
- **Skriveni kontakt (nikad na kartici):** `telefon` + `telefonSaglasnost` — vidi ga **samo UO**, preko `GET /api/admin/tabla-jemstva/neprepoznati` (spisak ljudi koje niko nije prepoznao; svaki pristup se loguje `PRISTUP_NEPREPOZNATI_JEMSTVO`); `linkProfila` — otkriva se **tek onome ko je odgovorio DA**. Oba se brišu pri verifikaciji, isteku i uklanjanju (minimizacija). **Ranije polje `kontaktPodaci` i ruta `/[id]/kontakt` su UKLONJENI** iz forme, prikaza i API-ja; kolona ostaje u bazi jedan ciklus radi povratka.
...
- **⚠️ Prepoznavanje NIJE jemstvo.** Odgovor „DA" otvara SAMO `Konverzacija` (+ link). Formalni čin jemstva je zaseban akt sa zasebnom potvrdom (`/[id]/verifikuj`) — ne spajati ni u UI ni u bazi.
