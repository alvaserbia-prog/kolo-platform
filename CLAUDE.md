...
- **Šta neverifikovani SME:** oglas tipa **PONUDA** (ne i POTRAZNJA), najviše **3 aktivna**, uz **sadržinski minimum** (bar 1 fotografija, kategorija, mesto — najmanja dužina opisa je UKINUTA 2026-08-13 odlukom vlasnika; ranije 40 znakova). Sme da **odgovara** u razgovoru koji je verifikovani pokrenuo povodom njegovog oglasa — mehanizam je već postojao za tablu i preuzeo je njen posao bez ijedne nove linije (`POST /api/poruke` traži verifikaciju, `POST /api/poruke/[konvId]` proverava samo članstvo).
