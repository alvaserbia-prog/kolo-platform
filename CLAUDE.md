...
**✅ Rendering app-a (od 2026-08-09):** javne pravne stranice čitaju iz **`dokumentacija 4.1/`** (loader `src/lib/pravni-dokument.ts`, baza = `dokumentacija 4.1`). Prikazuju se verzije **4.2.2** za sve akte (statut **4.1**). Prevodi celog seta su u **`dokumentacija 4.1/en/`**, **`/ru/`**, **`/hr/`** i **`/hu/`** (po 15 dokumenata × 4 jezika, uz disklejmer da je merodavan srpski original; loader bira prevod po locale-u, tih fallback na srpski ako fajl nedostaje). **hr i hu dodati 2026-08-09** — do tada su ta dva locale-a tiho dobijala srpski tekst jer ih loader nije mapirao; sada je set potpun na svih 5 jezika iz `src/i18n/routing.ts`. Integritet seta čuva test `__tests__/pravni-dokumenti.test.ts` (postojanje svih 15 akata po jeziku, prisustvo odredaba 4.2.2, odsustvo ukinutih instituta, disklejmer, i provera da se reči jednog jezika ne provuku u drugi). Dodatno su linkovani i **Pravilnik o Gornjem Kolu** i **Pravilnik o programima podrške** (slug `gornje-kolo`, `programi-podrske`). `nova dokumentacija/` je sada samo istorija.
...
8. **Kanali evidentiranja POEN-a (Pravilnik čl. 15 — osam kanala)**:
   ...
   - **Ne ulaze u dnevni limit (automatski akt Protokola):** verifikacija u lancu potvrda (dokaz stvarnosti), finansijski doprinos (donacije), pokroviteljstvo, rast kolektivnih oblika (bonus Kruga), osnivački doprinos, **doprinos sadržaju platforme (čl. 40a — osmi kanal, od 2026-08-09)**.
...
- **🔴 ČEKANJA VIŠE NEMA — doprinos se evidentira SVAKOME u trenutku objave (čl. 40a st. 3, odluka vlasnika 2026-08-11).** Vidi zasebnu sekciju „Doprinos za prvi oglas ide odmah, i bez potvrde" ispod. Tekst koji sledi opisuje **zatečeno** stanje do te izmene i drži se zbog zatečenih redova u bazi:
...
- 🔴 **Brane protiv naduvavanja opticaja UKLONJENE (odluka vlasnika, 2026-08-09)** — plan je predviđao dve (ručno okidanje osnivačkog koraka + admin alert na obrazac okidača); obe su uklonjene istog dana kad su i objavljene. Poslednja preostala — **razdvajanje BELEŽENJA od EVIDENTIRANJA** — uklonjena je 2026-08-11 (vidi sekciju ispod). Od brana ostaju samo **sadržinski minimum** oglasa i **najviše 3 aktivna oglasa** za nalog bez potvrde.
...
### Doprinos za prvi oglas ide ODMAH, i bez potvrde (2026-08-11)

Odluka vlasnika: nepotvrđen član treba da dobije **1.000 POEN odmah kad postavi oglas**, ne po nastupanju okidača. Time pada razlika koja je od 2026-08-09 bila „pravno srce" osmog kanala.

- **Šta se menja:** `zabeleziDoprinos()` više **ne čita tip naloga** — po upisu reda odmah zove `probajEvidentirati(userId, DoprinosOkidac.OBJAVA)`. Isto i za korake 2–5 putanje razmene (`probajNapredovati()`): pređen korak se evidentira odmah, bez obzira na potvrdu.
- **Enum je PREIMENOVAN, ne dopunjen:** `OBJAVA_VERIFIKOVAN` → **`OBJAVA`** (migracija `20260811130000_doprinos_objava_svima`). `ALTER TYPE … RENAME VALUE` je transakcion — za razliku od `ADD VALUE`, koja se ne sme koristiti u transakciji u kojoj je dodata (zbog čega su migracije iz avgusta razdvajane). Zatečeni redovi zadržavaju tačno značenje: i oni su nastali objavom.
- **`ZABELEZEN` više ne nastaje redovnim putem.** Ostaje kao (a) prelazno stanje unutar `probajEvidentirati()` — rezervacija pre emisije i povratak ako emisija pukne, i (b) zatečeni redovi iz vremena čekanja. Pozivi `probajEvidentirati`/`probajEvidentiratiKorake` iz verifikacije i `/api/transfer` **ostaju** — sada su mreža ispod, ne glavni put.
- **Zatečeni neverifikovani se razrešavaju DUGMETOM**, ne migracijom (emisija mora kroz `emitujPoen` — zero-sum, audit; opticaj skače za 1.000 × broj razrešenih i može da upali osnivački korak, pa taj trenutak bira čovek). `evidentirajZatecene()` više nikoga ne preskače; polje `preskocenoNeverifikovanih` je izbačeno iz odgovora i iz admin poruke.
- 🟡 **Izloženost je svesno prihvaćena.** Pedeset praznih naloga sa po jednim oglasom sada jesu 50.000 POEN u opticaju, a opticaj okida osnivački korak (24.000 POEN na svakih 100.000) i gasi prelazno ograničenje iz čl. 22 dokaza stvarnosti. Ostaju samo **sadržinski minimum** (fotografija, opis ≥ 40 znakova, kategorija, mesto) i **najviše 3 aktivna oglasa** bez potvrde. Ako naduvavanje počne da se dešava, rešenje NIJE tiho vraćanje čekanja nego razgovor sa vlasnikom.
- **Akti (4.2.2):** čl. 40a Pravilnika — tri stava (verifikovani / neverifikovani / obrazloženje razlike) zamenjena su jednim („Doprinos se evidentira u trenutku objave oglasa, nezavisno od toga da li je stvarnost korisnika potvrđena") uz stav koji izričito kaže da se ranije razlikovanje ne primenjuje; dodato da se **već evidentiran doprinos uklanjanjem oglasa ne poništava** (kod je to i radio); prelazni stav proširen na sve zatečene. Uz to čl. 40b (tačka 1 i stav o pređenom koraku), čl. 67 (upućivanje na „st. 4" koji više ne postoji) i **Uslovi čl. 16**. Sve na sr + en/ru/hr/hu.
- **Test `pravni-dokumenti.test.ts`** više ne traži „Verifikovanom korisniku doprinos se evidentira" nego novu rečenicu — ista uloga: da se odloženo evidentiranje ne vrati u akt mimo odluke vlasnika. Rečenica se traži na sr, en i ru.
- **Copy:** Pijaca (`neverif_opis`) više ne kaže „evidentira se kad te neko potvrdi"; korak 1 putanje u Novčaniku je sada samo prvi oglas (ranije „i prva razmena u kojoj ti neko prepiše POEN"). 🟡 **Usput ispravljeno u onboardingu** (`dobrodosli`, zatečene greške): `ekran5_p3` je znao za dva izvora POEN-a umesto tri, a `ekran6_p5` je tvrdio da se postavljanje oglasa otključava potvrdom — što ne važi od 4.1.0.
