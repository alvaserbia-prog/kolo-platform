...
- **„Viđeno" badge-evi (Početna, Novčanik, Pijaca):** broje stavke nastale POSLE poslednjeg otvaranja taba. Kolone `User.vidjenoNovcanikAt` / `vidjenoPijacaAt` / `vidjenoPocetnaAt` (migracije `20260611120000_sidebar_vidjeno`, `20260703120000_pocetna_vidjeno`); `GET /api/dnevni-brojevi` broji `createdAt > viđeno` (fallback ponoć ako tab nije otvaran); `POST /api/dnevni-brojevi/vidjeno {sekcija}` postavi „viđeno = sad" → badge na 0. Nulovanje okida `AppShell` `useEffect` na promenu `pathname` (`/pocetna` | `/novcanik` | `/pijaca`): optimističko nulovanje + POST + re-fetch. **Početna** broji nove poruke u Pričaonici (tuđe `ChatMessage`) + nove objave Fondacije (`BlogPost.publishedAt`) od `vidjenoPocetnaAt`. **`/api/me` sada računa `dnevniBrojevi` za SVE prijavljene** (ranije samo verifikovani), pa i neverifikovani vide Početna/Novčanik/Pijaca badge.
