...
async function proveriPristup() {
  ...
  if (!session) return { error: "Nije prijavljen.", status: 401 } as const;
  if (!jeSuperadmin(session.user)) return { error: "Samo admin.", status: 403 } as const;

  const kanal = await dohvatiIliKreirajKanal();
  if (kanal.brojKoraka > 0) {
    return { error: "Kanal je vec aktiviran (broj koraka > 0). Izmena liste osnivaca nije dozvoljena.", status: 409 } as const;
  }
  if (kanal.osnivaciZakljucani) {
    return { error: "Lista osnivača je zaključana. Prvo je otključajte.", status: 409 } as const;
  }
  return null;
}

export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const gr = await proveriPristup();
  if (gr) return NextResponse.json({ error: gr.error }, { status: gr.status });
