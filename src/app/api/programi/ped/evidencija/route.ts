...
// POST /api/programi/ped/evidencija — unos za danas
export async function POST(req: NextRequest) {
  ...
  if (!enrollment || enrollment.status !== "ACTIVE")
    return NextResponse.json({ error: "Niste aktivni na programu Evidencija doprinosa." }, { status: 403 });

  // Program mora biti aktivan
  const program = await prisma.protokolProgram.findUnique({ where: { type: "PED" } });
  if (!program?.isActive)
    return NextResponse.json({ error: "Program Evidencija doprinosa nije aktivan." }, { status: 400 });
