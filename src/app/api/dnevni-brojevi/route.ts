...
export async function GET() {
  ...
  const [wallet, pijaca, zajednica, zaposljavnje, programi, zrno] = await Promise.all([
    ...
    prisma.zadrugaPristupnica.count({ where: { createdAt: { gte: danas } } }),
    prisma.radniOglas.count({ where: { createdAt: { gte: danas }, status: "ACTIVE" } }),
  ...
  return NextResponse.json({ novcanik, pijaca, zajednica, zaposljavnje, programi, zrno });
}
