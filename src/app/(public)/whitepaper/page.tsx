...
export default async function WhitepaperPage() {
  ...
  const t = await getTranslations("pravne");
  const sadrzaj = await ucitajPravniDokument("whitepaper_3_8_0.md", locale);

  return (
    <div className="max-w-[800px] mx-auto pb-16">

      <div className="mb-8">
        <p className="text-xs text-kolo-muted mb-1">{t("dokumentacija")}</p>
        <h1 className="text-2xl font-bold text-kolo-green-900" style={{ letterSpacing: "-0.02em" }}>
          {t("whitepaper.naslov")}
        </h1>
        <p className="text-sm text-kolo-muted mt-2">{t("verzija")} {t("whitepaper.ver")}</p>
        <div className="mt-4 flex gap-3 text-sm flex-wrap">
          <span className="text-kolo-muted">{t("viditeI")}</span>
          <Link href="/o-sistemu" className="text-kolo-green-700 hover:underline">{t("link.oSistemu")}</Link>
