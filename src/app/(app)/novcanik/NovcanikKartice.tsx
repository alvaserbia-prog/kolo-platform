...
export default function NovcanikKartice({ balance, pseudonim, memberHash, platiPseudonim, prefillIznos, prefillOpis, zabelezenDoprinos = 0, zabelezenePotvrdeMoje = 0, zabelezenePotvrdeTudje = 0, rezervisanoNabavka = 0, smeDaSalje = true, razlogZabrane = "neverifikovan", maloletan = false }: Props) {
  ...
  return (
    <>
      ...
      <div>
        ...
        {(zabelezenePotvrdeMoje > 0 || zabelezenePotvrdeTudje > 0) && (
          <div className="mt-3 rounded-2xl border border-kolo-border bg-white px-5 py-4">
            <div className="flex items-baseline justify-between gap-3">
              <p className="text-sm font-semibold text-kolo-text">{t("zabelezene_potvrde_naslov")}</p>
              <p className="text-lg font-bold tabular-nums text-kolo-green-700">
                {(zabelezenePotvrdeMoje + zabelezenePotvrdeTudje).toLocaleString(intlTag(locale))} {tc("poen")}
              </p>
            </div>
