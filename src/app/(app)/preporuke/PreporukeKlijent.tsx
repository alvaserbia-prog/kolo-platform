  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl card-shadow border border-kolo-border p-6">
        <p className="text-sm font-semibold text-kolo-text mb-4">Vaš link za pozivanje</p>
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="w-full">
            ...
            <p className="text-xs text-kolo-muted mt-2 text-center">
              Podelite ovaj link ili QR kod — isti je za sve pozivnice i primanja POENA.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl card-shadow border border-kolo-border p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-sm font-semibold text-kolo-text">Vaš rang preporuka</p>
            <p className="text-3xl font-bold text-kolo-green-700 mt-1">Rang {data.trenutniRang}</p>
            <p className="text-xs text-kolo-muted mt-0.5">{data.verifikovani} verifikovanih preporučenih</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-kolo-muted">Bonus za sledeću</p>
          ...
          return (
            <div className="mt-4">
              <div className="flex justify-between text-xs text-kolo-muted mb-1">
                <span>Rang {data.trenutniRang}</span>
                <span>Rang {data.trenutniRang + 1} {preostalo ? `(još ${preostalo})` : ""}</span>
              </div>
          ...
          <table className="w-full min-w-[280px] text-sm">
            <thead>
              <tr className="bg-kolo-bg border-b border-kolo-border">
                ...
                <th className="px-4 py-2 text-left text-xs font-medium text-kolo-muted">Preporučenih</th>
                <th className="px-4 py-2 text-right text-xs font-medium text-kolo-muted">POEN/preporuka</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  ...
                  >
                    <td className={`px-4 py-2.5 font-medium ${active ? "text-kolo-green-900" : "text-kolo-text"}`}>
                      {r.rang === 0 ? "0" : `${r.rang}`}
                      {active && <span className="ml-2 text-xs text-kolo-green-700 font-semibold">← trenutni</span>}
                    </td>
      ...
      <div>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-base font-semibold text-kolo-text">Moje preporuke ({totalPreporučenih})</h2>
        </div>
        {data.preporuke.length === 0 ? (
          <div className="bg-white rounded-2xl card-shadow border border-kolo-border p-6 text-center text-sm text-kolo-muted">
            Još nema preporučenih članova. Podelite vaš link!
          </div>
        ) : (
          <div className="bg-white rounded-2xl card-shadow border border-kolo-border overflow-hidden">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-kolo-muted mt-0.5">
                    Registrovan: {new Date(p.registrovanAt).toLocaleDateString("sr-RS")}
                  </p>
                </div>
                <div className="text-right">
                  <span
