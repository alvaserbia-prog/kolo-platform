...
export default async function OpengraphImage({
  ...
  const listing = await prisma.marketplaceListing.findUnique({
    where: { id },
    select: { title: true, images: true, seller: { select: { maloletan: true } } },
  });
  ...
  if (!listing || listing.seller.maloletan) return brendiranaKartica("Pijaca");

  const naslov = listing.title.slice(0, 120);
