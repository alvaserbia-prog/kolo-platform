...
export async function izvrsiNocnuEmisiju(datum: Date) {
  ...
  // 2. Aktivni programi
  const aktivniProgrami = await prisma.bankaProgram.findMany({ where: { isActive: true } });
  ...
  if (aktivniTipovi.has("ZAPOSLJAVNJE")) {
    ...
    for (const ev of evidencije) {
      ...
      items.push({ walletId: ev.user.wallet.id, amount: ev.amount, type: "ZAPOSLJAVNJE", evidencijaId: ev.id });
    }
  }
