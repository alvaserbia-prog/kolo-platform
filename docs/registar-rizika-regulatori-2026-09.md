...
| **R-08** | ~~Strukturna hiperinflacija POEN-a uz objavljen odnos 1:1~~ → **opis povučen 17.09.2026.** Stvaran nalaz: kad je dnevni limit zasićen zapis je MANJI od doprinosa a ekran obećava „fiksan" iznos; whitepaper netačno opisuje sopstveni mehanizam; granicu količine postavlja tip kolone (`INTEGER`), ne pravilo | Zaštita potrošača / sistemski | **8** | ✅ obrađen 17.09.2026. → **3** |
...
| **R-08** | 16–17.09.2026. | Obrađen i sproveden. 🔴 **Opis rizika u tabeli je bio netačan i povučen je:** „hiperinflacija uz objavljen odnos 1:1" — odnos je uklonjen setom 4.5.8 (R-01), a rast koji prati broj učesnika i njihovu uključenost nije inflacija; uz to nije ni jednosmeran (pet osnova poništenja). Stvaran nalaz ide u SUPROTNOM smeru: kad je dnevni limit zasićen, zapis je MANJI od doprinosa, a ekran je obećavao „fiksan" iznos (pri opticaju 200.000 i sto korisnika — 200 umesto 2.000). 🟡 Odluka od 16.09. (POEN po potvrdi čeka prvi doprinos) taj nalaz **pojačava**, jer dodatno usporava rast opticaja u lansirnoj fazi. Odobreno M-1 (copy prestaje da obećava fiksan iznos, uz oba kanala), M-2 (whitepaper: „štiti od inflatornog pritiska" → tačan opis, dva mesta × 5 jezika), M-3 (briše se „Iznos nema gornju granicu" — protivrečila čl. 5 istog akta; bez zamene), M-4 (FAQ 300 „Tempo nije proizvoljan" → ko ga određuje u Fazi 1; nov odeljak `sporno3` na `/pravna-pozicija` × 5 jezika), M-5 (osnivački se ne dira) i M-6 u **izmenjenom obimu**. Odbijeno: kapa na predloženi POEN (operativni deli isti krov od 10%), prvenstvo socijalnih programa („operativni je prioritet"), drugi uslov za Fazu 2 (nosilaca ZRNA već ima pet, i to su osnivači — predmet R-10). Set **4.6.6**. Brana `r08-kolicina-poena-izvor.test.ts` (28 provera). | 8 → **3** |
| **R-08** (M-6) | 17.09.2026. | 🔴 **Nalaz koji rizik nije predvideo: granicu količine POEN-a postavlja TIP KOLONE.** `Wallet.balance` je `INTEGER`, pa je opticaj tvrdo ograničen na **2.147.483.647** — pri milion članova oko 2.147 POEN-a po čoveku, dakle dve potvrde. Nijedan `BigInt` u šemi, nijedna provera prekoračenja u `src/`. 🔴 Prelazak na `BIGINT` **nije urađen**: prvobitna procena („migracija od nekoliko milisekundi") bila je pogrešna — Prisma `Int → BigInt` menja JS tip u `bigint` i obara `Math.abs`, mešanu aritmetiku i `JSON.stringify` na **158 dodirnih tačaka**, među njima zero-sum jezgro. Umesto toga uzbuna na 80% granice i razumljiv opis greške (`src/lib/protokol/granica-zapisa.ts`); uzbuna ide iz noćne emisije jednom dnevno, ne iz `emitujPoen`. **Ostaje za poseban posao.** | — |
