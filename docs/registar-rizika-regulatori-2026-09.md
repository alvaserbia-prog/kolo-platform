...
| **R-08** | **Strukturna hiperinflacija POEN-a uz objavljen odnos 1:1** — dnevni limit je 10% opticaja koji sam raste tim emisijama; socijalni programi bez gornje granice; osnivački kanal se prazni u nedeljama | Zaštita potrošača / sistemski | **8** | 🟢 **umanjeno 16.09.2026.** (set 4.6.5) — ocena se preračunava pri obradi |
...
| **R-11** | **Prepis POEN-a kao platna usluga / POEN kao elektronski novac** | NBS | **7** | 🔴 **ulazi u obradu sa IZMENJENOM polaznom tačkom:** prepis je od 23.09.2026. otvoren i članu sa utvrđenim identitetom (set 4.6.6), pa se krug lica koja iniciraju ažuriranje evidencije više ne poklapa sa krugom potvrđenih. To je prvi podatak njegove obrade |
| **R-12** | **Sprečavanje pranja novca** — kontrola uplatioca ne radi kod kartice i IPS-a, zabrana prodaje POEN-a bez detekcije, otvorena petlja donacija→POEN→roba, trostrana konstrukcija pokroviteljstva | APML | **7** | 🔴 **otvaranje prepisa (4.6.6) obara R22-M2** („uplatilac kao ključ za duplikat", najjeftinija detekcija koju imamo): punjenje više naloga više ne traži više uplatilaca nego jednu donaciju pa raspodelu iznutra |
...
| **R-16** | **Čitanje osnivačkog kanala kao lančane šeme** — rast koji finansiraju novi učesnici automatski puni zatvoren krug koji je ušao prvi | Zaštita potrošača | **7** | 🟢 **umanjeno 16.09.2026.** (set 4.6.5) — ocena se preračunava pri obradi |
...
| **R-03** (4.6.7) | 25.09.2026. | 🔴 **Mera M-1 OBRNUTA odlukom vlasnika: evidentiranje po socijalnom programu prikazuje se redovnim članovima** (pseudonim, naziv programa, iznos), uz obrazloženje da produktivni članovi treba da mogu da provere emisiju svakog člana. Gostu i novom članu ostaje dnevni zbir; dan sa jednim korisnikom se i dalje preskače. Program `POSEBNA_BRIGA` na ekranu je **Posebna podrška** i dobija dva osnova (smanjena sposobnost — rešenje o stepenu invaliditeta uključujući komisiju za procenu radne sposobnosti, odnosno bolest uz branu „ne može ili je bitno ograničen da učestvuje”; i gubitak doma, u kome je prinudna raseljenost namerno spojena sa nepogodom da ne oda nacionalnu pripadnost). 🔴 Nosiva mera nije više izostavljanje zapisa nego **izostavljanje OSNOVA** — osnov se ne prikazuje nijednom korisniku. Kumulacija sa Podrškom majkama izričito dopuštena. Set **4.6.7** (programi podrške, Politika, Registar radnja 10, DPIA × 5 jezika), Pravilnik ostao na 4.6.6. Migracija `20260925120000_pristanak_4_6_7` (ponovna saglasnost). 🔴 **Preostaje u kodu:** `Transaction` ne nosi vezu do prijave, pa prikaz naziva programa još ne postoji; `ProgramEnrollment` ne nosi osnov, pa trajanja po osnovu nisu sprovedena. Do tada se set NE objavljuje. | 4 → **6** (DPIA R11 6 → 9, srednji nivo, gornja granica) |
| **R-01** (4.6.6) | 23.09.2026. | 🔴 **Prepis POEN-a OTVOREN članu sa utvrđenim identitetom** (odluka vlasnika). Obrazloženje: zabrana nije sprečavala ishod nego samo redosled — oglašivač sme da potvrdi kupca na istom sastanku, pa se isti prepis izvršavao posle potvrde; uz to je POEN identifikovanom članu imao tačno jednu upotrebu (ZRNO), pa je zabrana trošenja bila podsticaj gomilanju. Zatvoreni ostaju aktiviranje i otpis ZRNA, glas i delegiranje, potvrđivanje drugih, nadzor, operativni doprinos, programi, nabavka, pokroviteljstvo, kontakt oglašivača. Set **4.6.6** (Pravilnik, Uslovi × 5 jezika); whitepaper provera negativna. Kod: `smeDaSalje` prima i `identitetUtvrdjen`, P-2 usklađen, ekran POEN više ne prepisuje pravilo. | **5 → 9** po tabeli registra; **6–7** po proceni sesije (obrazloženje u `docs/sprovodjenje-rizika-2026-09.md`) |
...
| **odluka vlasnika** | 16.09.2026. | Nije rizik iz registra nego projektantska izmena, upisana ovde jer dodiruje pet rizika. POEN po potvrdi (1.000 + 1.000) više ne nastaje u trenutku potvrde nego kad potvrđeni korisnik ostvari **prvi potvrđen doprinos**: odobren prvi oglas, javna donacija, pokroviteljstvo ili verifikovan operativni doprinos — sva četiri prolaze kroz ljudsku odluku i nijedan se ne može sam sebi izdati. Prepis POEN-a, osnivački i socijalni programi su izričito isključeni. Sam čin potvrde se ne menja: indeks i pun pristup nastaju odmah. Uz to svaki prvi oglas iz čl. 40a od sada ide na odobrenje, a okidači `VERIFIKACIJA` i `PRIMLJEN_POEN` su uklonjeni (bez toga bi odobrenje bilo šupljikavo i nastao bi krug). Set **4.6.4** (dokaz stvarnosti, Pravilnik × 5 jezika). Migracije `20260916120000/120100/130000/130100`. Brane `potvrda-uslov.test.ts` i `potvrda-uslov-izvor.test.ts`. 🔴 **Ocene se ne menjaju** — R-08 i R-16 nisu obrađeni, pa se preračunavaju kad dođu na red; ovde se beleži samo da im se zatečeno stanje popravilo. | R-08, R-16, R-22 🟢 · R-02, R-07 🔴 pažnja u tekstu |
...
| sve mere + puna C (prepis otvoren) | 9 | 🔴 **SPROVEDENO 23.09.2026** (odluka vlasnika, set 4.6.6). Ranije je ovde stajalo „odbijeno". Procena sesije je **6–7**, jer je element „prihvataju kao sredstvo razmene" bio ispunjen i pre izmene, a lanac *kupljeno → preneto* zatvoren već za potvrđenog donatora — opcija B ga je odlagala za jednu potvrdu, ne prekidala. Obe ocene se vode, ne bira se između njih bez naloga |
